...
//
// Remember: all element of PageRequest should be READ ONLY! once it changed, the logic of the query may broke
func PageQuery(page int, rawPerPage int, queryHandler *gorm.DB, resultPtr interface{}) (*PageRequest, error) {
	//recovery
	defer recovery()
	count := 0
...
func (r *PageRequest) SetHandler(handler *gorm.DB) {
	r.handler = handler
}

// GetNextPage: return next page`s PageRequest
// 	func getResultSet (page int,rowsPerPage int)(*pageable.PageRequest,error){
...
// 		}
//		//get next page
//		resp,err := resp.GetNextPage()	//PageRequest of next page
// 	}
func (r *PageRequest) GetNextPage() (*PageRequest, error) {
	return PageQuery(r.PageNow+1, r.RawPerPage, r.handler, r.ResultSet)
}

// GetNextPage: return last page`s PageRequest
func (r *PageRequest) GetLastPage() (*PageRequest, error) {
	return PageQuery(r.PageNow-1, r.RawPerPage, r.handler, r.ResultSet)
}

// GetNextPage: return end page`s PageRequest
func (r *PageRequest) GetEndPage() (*PageRequest, error) {
	return PageQuery(r.PageCount, r.RawPerPage, r.handler, r.ResultSet)
}

// GetFirstPage: return first page`s PageRequest
func (r *PageRequest) GetFirstPage() (*PageRequest, error) {
	p := 1
	if use0Page {
		p = 0
	}
