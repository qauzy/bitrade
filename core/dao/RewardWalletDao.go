...
type RewardWalletDao interface {
	...
	FindAllByMemberId(memberId int64) (result []entity.RewardWallet, err error)
	Save(m *entity.RewardWallet) (result *entity.RewardWallet, err error)
	FindById(id int64) (result *entity.RewardWallet, err error)
	DeleteById(id int64) (count int64, err error)
	FindAll(qp *types.QueryParam) (result []*entity.RewardWallet, err error)
}
...
func (this *rewardWalletDao) FindAllByMemberId(memberId int64) (result []entity.RewardWallet, err error) {
	err = this.DBRead().Where("member_id = ?", memberId).Find(&result).Error
	return
}
func (this *rewardWalletDao) Save(m *entity.RewardWallet) (result *entity.RewardWallet, err error) {
...
func (this *rewardWalletDao) FindAll(qp *types.QueryParam) (result []*entity.RewardWallet, err error) {
	d := this.DBRead()
	if qp != nil {
		d = qp.BuildQuery(d)
	}
