...
type IeoEmptionDao interface {
	...
	FindAll(qp *types.QueryParam) (result []*entity.IeoEmption, err error)
}
...
func (this *ieoEmptionDao) FindbyCondition(id int64, times string) (result entity.IeoEmption, err error) {
	...
	//FIXME 非原生sql，需要处理
	eng := this.DBWrite().Exec("SELECT * from ieo_emption where id=? and start_time<=? AND end_time>?", id, times)
	err = eng.Error
	return
}
...
func (this *ieoEmptionDao) FindAll(qp *types.QueryParam) (result []*entity.IeoEmption, err error) {
	d := this.DBRead()
	if qp != nil {
		d = qp.BuildQuery(d)
	}
