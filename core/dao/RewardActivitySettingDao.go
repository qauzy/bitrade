...
type RewardActivitySettingDao interface {
	...
	FindAll(qp *types.QueryParam) (result arraylist.List[*entity.RewardActivitySetting], err error)
}
...
func (this *rewardActivitySettingDao) FindAll(qp *types.QueryParam) (result arraylist.List[*entity.RewardActivitySetting], err error) {
	d := this.DBRead()
	if qp != nil {
		d = qp.BuildQuery(d)
	}
