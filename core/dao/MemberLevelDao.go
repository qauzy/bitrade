...
type MemberLevelDao interface {
	...
	FindAll(qp *types.QueryParam) (result []*entity.MemberLevel, err error)
}
...
func (this *memberLevelDao) FindAll(qp *types.QueryParam) (result []*entity.MemberLevel, err error) {
	d := this.DBRead()
	if qp != nil {
		d = qp.BuildQuery(d)
	}
