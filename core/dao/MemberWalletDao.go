...
type MemberWalletDao interface {
	...
	FindAllByMemberId(memberId int64) (result []entity.MemberWallet, err error)
	FindAllByCoin(coin entity.Coin) (result []entity.MemberWallet, err error)
	GetWalletAllBalance(coinName string) (result math.BigDecimal, err error)
	FindALLByCoinIdAndMemberIdList(coin entity.Coin, memberIdList []int64) (result []entity.MemberWallet, err error)
	FindCoinIdAndMemberId(coinId string, memberId int64) (result entity.MemberWallet, err error)
	CreateGiftTable(times int64, coinId string) (result int64, err error)
	FindGiftTable(times int64) (result []entity.MemberWallet, err error)
	...
	FindAll(qp *types.QueryParam) (result []*entity.MemberWallet, err error)
}
...
func (this *memberWalletDao) ThawBalance(walletId int64, amount math.BigDecimal) (result int, err error) {
	...
	//FIXME 非原生sql，需要处理
	eng := this.DBWrite().Exec("update MemberWallet wallet set wallet.balance = wallet.balance + ?,wallet.frozenBalance=wallet.frozenBalance - ? where wallet.id = ? and wallet.frozenBalance >= :amount", walletId, amount)
	err = eng.Error
	return
}
func (this *memberWalletDao) FreezeBalance(walletId int64, amount math.BigDecimal) (result int, err error) {
	...
	//FIXME 非原生sql，需要处理
	eng := this.DBWrite().Exec("update MemberWallet wallet set wallet.balance = wallet.balance - ?,wallet.frozenBalance=wallet.frozenBalance + ? where wallet.id = ? and wallet.balance >= :amount", walletId, amount)
	err = eng.Error
	return
}
...
func (this *memberWalletDao) FindAllByMemberId(memberId int64) (result []entity.MemberWallet, err error) {
	err = this.DBRead().Where("member_id = ?", memberId).Find(&result).Error
	return
}
func (this *memberWalletDao) FindAllByCoin(coin entity.Coin) (result []entity.MemberWallet, err error) {
...
func (this *memberWalletDao) GetWalletAllBalance(coinName string) (result math.BigDecimal, err error) {
	...
	//FIXME 非原生sql，需要处理
	eng := this.DBWrite().Exec("select ifnull(sum(a.balance),0)+ ifnull(sum(a.frozen_balance),0) as allBalance from member_wallet a where a.coin_id = ?", coinName)
	err = eng.Error
	return
}
func (this *memberWalletDao) FindALLByCoinIdAndMemberIdList(coin entity.Coin, memberIdList []int64) (result []entity.MemberWallet, err error) {
...
func (this *memberWalletDao) FindCoinIdAndMemberId(coinId string, memberId int64) (result entity.MemberWallet, err error) {
	...
	//FIXME 非原生sql，需要处理
	eng := this.DBWrite().Exec("select * from member_wallet  where coin_id=? and member_id=?", coinId, memberId)
	err = eng.Error
	return
}
func (this *memberWalletDao) CreateGiftTable(times int64, coinId string) (result int64, err error) {

	//FIXME 非原生sql，需要处理
	eng := this.DBWrite().Exec("CREATE TABLE member_wallet_? SELECT * FROM member_wallet where coin_id=? AND balance>0"
	" AND is_lock=0", times, coinId)
	err = eng.Error
	return
}
func (this *memberWalletDao) FindGiftTable(times int64) (result []entity.MemberWallet, err error) {
...
func (this *memberWalletDao) ReleaseReisterGiving(amount math.BigDecimal, id int64) (result int, err error) {
	...
	//FIXME 非原生sql，需要处理
	eng := this.DBWrite().Exec("update MemberWallet wallet set wallet.balance=wallet.balance + ?,wallet.releaseBalance= wallet.releaseBalance -? where wallet.id = :walletId and wallet.releaseBalance >= :amount", amount, id)
	err = eng.Error
	return
}
...
func (this *memberWalletDao) FindAll(qp *types.QueryParam) (result []*entity.MemberWallet, err error) {
	d := this.DBRead()
	if qp != nil {
		d = qp.BuildQuery(d)
	}
