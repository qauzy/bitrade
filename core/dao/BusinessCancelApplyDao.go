...
type BusinessCancelApplyDao interface {
	...
	FindAll(qp *types.QueryParam) (result arraylist.List[*entity.BusinessCancelApply], err error)
}
...
func (this *businessCancelApplyDao) FindByMemberAndStatusOrderByIdDesc(member *entity.Member, status *CertifiedBusinessStatus.CertifiedBusinessStatus) (result arraylist.List[entity.BusinessCancelApply], err error) {
	err = this.DBRead().Where("id_desc = ?", member).First(&result).Error
	return
}
func (this *businessCancelApplyDao) FindByMemberOrderByIdDesc(member *entity.Member) (result arraylist.List[entity.BusinessCancelApply], err error) {
	err = this.DBRead().Where("id_desc = ?", member).First(&result).Error
	return
}
...
func (this *businessCancelApplyDao) FindAll(qp *types.QueryParam) (result arraylist.List[*entity.BusinessCancelApply], err error) {
	d := this.DBRead()
	if qp != nil {
		d = qp.BuildQuery(d)
	}
