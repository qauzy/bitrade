...
type ReleaseBalanceDao interface {
	...
	FindAll(qp *types.QueryParam) (result []*entity.ReleaseBalance, err error)
}
...
func (this *releaseBalanceDao) FindAll(qp *types.QueryParam) (result []*entity.ReleaseBalance, err error) {
	d := this.DBRead()
	if qp != nil {
		d = qp.BuildQuery(d)
	}
