...
type MemberAddressDao interface {
	...
	FindAllByMemberIdAndAddressAndStatus(memberId int64, address string, status CommonStatus.CommonStatus) (result []entity.MemberAddress, err error)
	FindByMemberIdAndCoinAndAddressAndStatus(memberId int64, coin entity.Coin, address string, status CommonStatus.CommonStatus) (result []entity.MemberAddress, err error)
	...
	FindAll(qp *types.QueryParam) (result []*entity.MemberAddress, err error)
}
...
func (this *memberAddressDao) FindAllByMemberIdAndAddressAndStatus(memberId int64, address string, status CommonStatus.CommonStatus) (result []entity.MemberAddress, err error) {
	err = this.DBRead().Where("member_id = ?", memberId).Where("address = ?", address).Where("status = ?", status).Find(&result).Error
	return
}
func (this *memberAddressDao) FindByMemberIdAndCoinAndAddressAndStatus(memberId int64, coin entity.Coin, address string, status CommonStatus.CommonStatus) (result []entity.MemberAddress, err error) {
...
func (this *memberAddressDao) FindAll(qp *types.QueryParam) (result []*entity.MemberAddress, err error) {
	d := this.DBRead()
	if qp != nil {
		d = qp.BuildQuery(d)
	}
