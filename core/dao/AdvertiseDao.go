...
type AdvertiseDao interface {
	FindAllByMemberIdAndStatusNot(var1 int64, status *AdvertiseControlStatus.AdvertiseControlStatus, var2 *domain.Sort) (result arraylist.List[entity.Advertise], err error)
	...
	FindAllByMemberIdAndStatusAndAdvertiseType(var1 int64, status *AdvertiseControlStatus.AdvertiseControlStatus, oType *AdvertiseType.AdvertiseType) (result arraylist.List[entity.Advertise], err error)
	...
	FindAllByMemberIdAndStatus(vari int64, status *AdvertiseControlStatus.AdvertiseControlStatus) (result arraylist.List[entity.Advertise], err error)
	AlterStatusBatch(status *AdvertiseControlStatus.AdvertiseControlStatus, updateTime time.Time, ids []int64) (result int, err error)
	...
	FindAll(qp *types.QueryParam) (result arraylist.List[*entity.Advertise], err error)
}
...
func (this *advertiseDao) FindAllByMemberIdAndStatusNot(var1 int64, status *AdvertiseControlStatus.AdvertiseControlStatus, var2 *domain.Sort) (result arraylist.List[entity.Advertise], err error) {
...
func (this *advertiseDao) FindAllByMemberIdAndStatusAndAdvertiseType(var1 int64, status *AdvertiseControlStatus.AdvertiseControlStatus, oType *AdvertiseType.AdvertiseType) (result arraylist.List[entity.Advertise], err error) {
...
func (this *advertiseDao) UpdateAdvertiseAmount(status *AdvertiseControlStatus.AdvertiseControlStatus, id int64, amount math.BigDecimal) (result int, err error) {
	...
	//FIXME 非原生sql,需要处理
	eng := this.DBWrite().Exec("update Advertise a set a.remainAmount=a.remainAmount-?,a.dealAmount=a.dealAmount+? where a.remainAmount>=? and a.status=:sta and a.id=?", status, id, amount)
	err = eng.Error
	return
}
...
func (this *advertiseDao) FindAllByMemberIdAndStatus(vari int64, status *AdvertiseControlStatus.AdvertiseControlStatus) (result arraylist.List[entity.Advertise], err error) {
	err = this.DBRead().Where("member_id = ?", vari).Where("status = ?", status).Find(&result).Error
	return
}
func (this *advertiseDao) AlterStatusBatch(status *AdvertiseControlStatus.AdvertiseControlStatus, updateTime time.Time, ids []int64) (result int, err error) {
...
func (this *advertiseDao) GetAdvertiseNum(member *entity.Member) (result int64, err error) {
	eng := this.DBWrite().Table("Advertise as a").Select("count(a.id)").Where("a.member = ?", member).Find(&result)
	err = eng.Error
	return
}
func (this *advertiseDao) UpdateAdvertiseDealAmountAndRemainAmount(id int64, amount math.BigDecimal) (result int, err error) {
	...
	//FIXME 非原生sql,需要处理
	eng := this.DBWrite().Exec("update Advertise a set a.dealAmount=a.dealAmount-?,a.remainAmount=a.remainAmount+? where a.dealAmount>=?  and a.id=?", id, amount)
	err = eng.Error
	return
}
...
func (this *advertiseDao) FindAll(qp *types.QueryParam) (result arraylist.List[*entity.Advertise], err error) {
	d := this.DBRead()
	if qp != nil {
		d = qp.BuildQuery(d)
	}
