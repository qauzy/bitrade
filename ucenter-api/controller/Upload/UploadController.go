...
func (this *UploadController) UploadOssImage(ctx *gin.Context, request *http.HttpServletRequest, response *http.HttpServletResponse, file *multipart.MultipartFile) (result *util.MessageResult, err error) {
	...
	var fileType = UploadFileUtil.GetFileType(file.GetInputStream())
	log.Info("fileType=" + fileType)
	var directory = SimpleDateFormat("yyyy/MM/dd/").Format(time.Now())
	var ossClient = OSSClient(this.AliyunConfig.GetOssEndpoint(), this.AliyunConfig.GetAccessKeyId(), this.AliyunConfig.GetAccessKeySecret())
	exception := func() (err error) {
		var fileName = file.GetOriginalFilename()
		var suffix = fileName.Substring(fileName.LastIndexOf("."), len(fileName))
		...
		var uri = this.AliyunConfig.ToUrl(key)
		var mr = MessageResult(0, this.SourceService.GetMessage("UPLOAD_SUCCESS"))
		mr.SetData(uri)
		return mr
		return
	}()
...
func (this *UploadController) UploadLocalImage(ctx *gin.Context, request *http.HttpServletRequest, response *http.HttpServletResponse, file *multipart.MultipartFile) (result *util.MessageResult, err error) {
	...
	var result = UploadFileUtil.UploadFile(file, fileName)
	if result != nil {
		var mr = MessageResult(0, this.SourceService.GetMessage("UPLOAD_SUCCESS"))
		mr.SetData(this.Host + result)
		return mr
	} else {
		var mr = MessageResult(0, this.SourceService.GetMessage("FAILED_TO_WRITE"))
		mr.SetData(result)
		return mr
	}
}
func (this *UploadController) Base64UpLoad(ctx *gin.Context, base64Data string, oss bool) (result *util.MessageResult) {
	...
	exception := func() (err error) {
		...
		if base64Data == nil || "".Equals(base64Data) {
			...
			var d = strings.Split(base64Data, "base64,")
			if d != nil && len(d) == 2 {
				dataPrix = d[0]
				data = d[1]
			} else {
		...
		var bs = Base64Utils.DecodeFromString(data)
		if oss == true {
			//阿里云上传
			var ossClient = OSSClient(this.AliyunConfig.GetOssEndpoint(), this.AliyunConfig.GetAccessKeyId(), this.AliyunConfig.GetAccessKeySecret())
			exception := func() (err error) {
				//使用apache提供的工具类操作流
				var is = ByteArrayInputStream(bs)
...
func NewUploadController(allowedFormat string, allowedVideo string, aliyunConfig *config.AliyunConfig, sourceService *service.LocaleMessageSourceService, host string) (ret *UploadController) {
	ret = new(UploadController)
	ret.AllowedFormat = allowedFormat
	ret.AllowedVideo = allowedVideo
	ret.AliyunConfig = aliyunConfig
