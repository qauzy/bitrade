...
func (this *CoinExchangeController) Exchange(ctx *gin.Context, member *transform.AuthMember, from string, to string, amount *math.BigDecimal) (result *MessageResult.MessageResult) {
	var coin, err = this.AssetExchangeService.FindOne(from, to)
	if err != nil {
		return MessageResult.Error(err.Error())
	}
	if coin == nil {
		return MessageResult.NewMessageResultV2(500, "不支持该币种兑换")
	}
	if amount.CompareTo(decimal.Zero) <= 0 {
		return MessageResult.NewMessageResultV2(500, "数量需大于0")
	}
